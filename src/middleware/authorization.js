...
export async function checkAuth(req, res, next) {
  try {
    ...
    return fail(res, 403, `Authenticated failed! ${err.message} pls login`);
  }
}
...
export function isValidEditor(req, res, next) {
  try {
    const { userType } = req.user;
    if (userType === USER_TYPE.EDITOR) return next();
    return fail(res, 403, "Invalid Editor credentials!");
  } catch (err) {
    // loging(module, req, err);
    return fail(res, 403, `User not Validated! ${err.message}`);
  }
