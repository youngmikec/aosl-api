...
import { response, success, fail } from '../../util/response.js';
  ...
  export const resetCodeVerificationHandler = async (req, res) => {
    try {
        const result = await resetCodeVerificationService(req.body);
        return success(res, 200, result);
    } catch (err) {
        return fail(res, 400, `${err.message}`);
    }
  ...
  export const userLoginHandler = async (req, res) => {
    try {
      ...
      if (!req.body.userType) req.body.userType = USER_TYPE.USER;
