...
export const loginService = async (loginPayload) => {
    ...
    if (userType === USER_TYPE.USER) {
      ...
      if ((password !== user.password) && !bcryptjs.compareSync(password, `${user.password}`)) {
  ...
  export const resetCodeVerificationService = async (data) => {
    try {
        ...
        const user = await Users.findOne({_id: id}).exec();
        if(!user) throw new Error(`User does not exist`);
        if(resetCode !== user.resetCode) throw new Error(`Forgot password code mismatch`);
  ...
  export const forgotPasswordService = async (data) => {
    try {
        ...
        const result = await Users.findOneAndUpdate({email: email}, {...payload}, { new: true}).exec();
        if(!result) throw new Error(`${module} Error trying to reset password`);
  ...
  export const passwordResetCodeService = async (email) => {
    try {
        ...
        const result = await Users.findOneAndUpdate({email: email}, {resetCode: resetCode}, { new: true}).exec();
