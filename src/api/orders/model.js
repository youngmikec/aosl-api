...
import { returnCrypto } from "../../util/helpers.js";
...
export const validateCreate = joi.object({
  orderType: joi
    .string()
    .trim()
    .valid(...Object.values(ORDERS.TYPES))
    .required(),
  ...
  walletAddress: joi.string().trim().optional().when("paymentMethod", {
    is: ORDERS.PAYMENT_METHOD.WALLET,
    then: joi.required(),
    otherwise: joi.forbidden(),
  }),
  bankName: joi.string().trim().optional().when("paymentMethod", {
    is: ORDERS.PAYMENT_METHOD.BANK,
    then: joi.required(),
    otherwise: joi.forbidden(),
  }),
  accountName: joi.string().trim().optional().when("paymentMethod", {
    is: ORDERS.PAYMENT_METHOD.BANK,
    then: joi.required(),
    otherwise: joi.forbidden(),
  }),
  accountNumber: joi.string().trim().optional().when("paymentMethod", {
    is: ORDERS.PAYMENT_METHOD.BANK,
    then: joi.required(),
    otherwise: joi.forbidden(),
  }),
  ...
  cryptocurrency: joi
    ...
    .optional()
    .when("orderType", {
      is: returnCrypto(ORDERS.TYPES.BUY_CRYPTO || ORDERS.TYPES.SELL_CRYPTO),
      then: joi.required(),
      otherwise: joi.optional(),
    }),
  // .when("orderType", {
  //   is: returnCrypto(ORDERS.TYPES.SELL_CRYPTO),
  //   then: joi.required(),
  //   otherwise: joi.forbidden(),
  // }),
  ...
  cardNumber: joi.string().trim().min(10).optional().when("orderType", {
    is: ORDERS.TYPES.GIFTCARD,
    then: joi.required(),
    otherwise: joi.forbidden(),
  }),
...
export const validateUpdate = joi.object({
  orderType: joi
    ...
    .valid(...Object.values(ORDERS.TYPES))
    .optional(),
  sendersPhone: joi.string().trim().optional().when("orderType", {
    is: ORDERS.TYPES.AIRTIME,
    then: joi.required(),
    otherwise: joi.forbidden(),
  }),
